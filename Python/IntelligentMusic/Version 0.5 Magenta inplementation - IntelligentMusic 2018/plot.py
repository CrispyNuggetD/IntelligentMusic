		...
		if plotOffsetBars == 0:
			...
			if normalise == 1:
				...
				if toPlotBefore == 1:
					...
					plt.axis([plotOffsetBars, bigOffsetDataPoint,-dataHeight,dataHeight])

		print ("Sound wave plotted!")
		print ("Plotting took", time.time() - start_time1, "seconds.")
		plt.show()
	else:
...
def plotFourier():
	...
	#plt.ion()
	if normalise == 1:
		plt.plot(x,np.abs(normalisedData))
	else:
		plt.plot(x,song)
