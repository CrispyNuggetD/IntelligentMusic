...
def GenerateNote(noteLength,freq,addfreq=[]):
	...
	localData = np.array(np.tan(dataPoints*(freq*(2*np.pi))))
		#make array; DO NOT Append to current data

	'''
	localData = np.array(signal.square(dataPoints*(freq*(2*np.pi))))
		#square

	localData = np.array(signal.sawtooth(dataPoints*(freq*(2*np.pi))))
		#sawtooth

	localData = np.array(signal.sawtooth(dataPoints*(freq*(2*np.pi)),0.5))
		#triangle

	localData = np.array(np.sin(dataPoints*(freq*(2*np.pi))))
		#sine
