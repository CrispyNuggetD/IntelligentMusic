...
def GenerateNote(freqComponents,noteData):
	...
	localData = "" 
		...
		####WRONG HERE BTW: LOCALDATA NEEDS TO BE AFTER NOTEDATA LOOP
FOR DATA TO NOT BE OVERWRITTEN BY SECOND NOTE
	...
	for noteData in noteData:
		...
		for multipleNotes in noteData[1]:
			print "ASDDS: " + str(multipleNotes)
			print "Generating melody (" + str(note(str(multipleNotes))) + "Hz) " + "(length: " + str(noteData[0]) + ")..."
			
			theFreq = note(str(multipleNotes)) 
				#given frequency
