...
'''
			...
			#make def evaluate which for loops the noteData array
def GenerateNote(freqComponents,noteData):
	...
	for noteData in noteData:
		print "Generating melody (" + str(note(str(noteData[1]))) + "Hz) " + "(length: " + str(noteData[0]) + ")..."
		...
		theFreq = note(str(noteData[1])) 
			#given frequency
		freqRatio = theFreq/freqComponents[0][0]
		
		for waveform in freqComponents:
			finalFreq = freqRatio*waveform[0]

			print "		Generating waveform (" + str(waveform[1]) + ", " + str(finalFreq) + "Hz)..."
			
			if waveform[1] == "sine":
			#addData refers to additional data
				addData = np.sin(dataPoints*finalFreq)
		###MIGHT BE WRONG note data??? Need to divide by base freq.
				#make array; DO NOT Append to current data
				#adddata to not overwrite original data
			elif waveform[1] == "square":
				addData = signal.square(dataPoints*finalFreq)
				#square
			elif waveform[1] == "sawtooth":
				addData = signal.sawtooth(dataPoints*finalFreq)
				#sawtooth
			elif waveform[1] == "triangle":
				addData = signal.sawtooth((dataPoints*finalFreq),0.5)
				#triangle
			if localData == "":
				localData = addData
			else:
